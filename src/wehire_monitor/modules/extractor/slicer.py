...
切片策略:
  - 单图高度 >3000px 且该图 OCR 文本 <300字 → 触发切片
  - 最大切片高度 1800px
  - 重叠 220px(避免文字被切断)
  - 原宽 >1440px 等比缩放到 1440px
  - 每篇最多 8 切片(超限则自适应增大 step 均匀覆盖)
...
# 保存时的 JPEG 质量(VLM 识别无需 95 高质量,85 足够且文件更小)
_JPEG_QUALITY = 85


def should_slice_image(image_path: str, ocr_text_len: int) -> bool:
    """触发条件: 单图高度 >3000px 且该图 OCR 文本 <300字

    Args:
        image_path: 图片本地路径
        ocr_text_len: **该图片**的 OCR 文本字符数(非全部图片合计)
...
class LongImageSlicer:
    ...
    Args:
        ...
        max_slices: 每张图最多切片数(默认 8,超限自适应 step)。
    """
    ...
    def slice_image(
        ...
        try:
            ...
            actual_sh = self.slice_height  # 实际使用的切片高度(可能自适应增大)

            # 计算所需切片数
            num_needed = math.ceil(height / step) if step > 0 else 1

            if num_needed > self.max_slices:
                # 超限:自适应增大 slice_height 以用 max_slices 均匀覆盖全图
                # n*sh - (n-1)*ov >= h → sh >= (h + (n-1)*ov)/n
                effective_sh = math.ceil(
                    (height + (self.max_slices - 1) * self.overlap) / self.max_slices
                )
                effective_sh = min(effective_sh, height)  # 不超过图高
                step = effective_sh - self.overlap
                actual_sh = effective_sh
                num = self.max_slices
                logger.info(
                    f"图片过高需 {num_needed} 片,自适应调整 slice_height={effective_sh}px "
                    f"(step={step})以用 {num} 片均匀覆盖"
                )
            else:
                num = max(1, num_needed)

            logger.info(f"切片数: {num} (step={step}, slice_height={actual_sh}, height={height})")

            # 4. 逐切片裁剪、保存
            slices: list[ImageSlice] = []
            for i in range(num):
                y_start = i * step
                y_end = min(y_start + actual_sh, height)
                # 最后一片确保到图底
                if i == num - 1:
                    y_end = height
                is_bottom = (i == num - 1)
                ...
                crop.save(out_path, "JPEG", quality=_JPEG_QUALITY)
                crop.close()  # 及时释放内存
                ...
                slices.append(ImageSlice(
                    pil_image=None,  # 不保留内存中的 PIL 对象(节省内存,通过 local_path 读取)
        ...
        # 5. 返回 ImageSlice 列表(已按 y_start 升序)
        return slices
